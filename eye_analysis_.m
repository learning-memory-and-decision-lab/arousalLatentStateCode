%eye_analysis('~/Dropbox (Brown)/arousalLearningPerception/vwm_task/ET_data/Meera022620.mat',...
                        %200, 4000, 150);
function result = eye_analysis_(path, before, after, blinkWindow,subNum)
...
blocks = data(data(:,event) == block, 1);
...
stimulusData = data((data(:,event) == stimOn), :);
...
% Getting the rows that have stimulus information for all 3 blocks
pred1Data = data((data(:,event) == predCue1on), :);
...
stimOff = 5;
stimOffEvents  = data(find(data(:,event) == stimOff & data(:,1) > stimStart(1)), 1);
...
% end
%% Finding start of each responseMade2
responseMade2 = 9;
responseMade2Events  = data(find(data(:,event) == responseMade2 & data(:,1) > stimStart(1)), 1);
endEst = [responseMade2Events(1,1)];
previous = responseMade2Events(1,1);
for i = 1:size(responseMade2Events, 1)
%     Sometimes there is a 0 within the stimulus, we need to skip it
    if responseMade2Events(i,1) - previous > 10 && i ~= 1
        endEst= [endEst; responseMade2Events(i, 1)];
    end
    previous = responseMade2Events(i,1);
end

endEst(50)=[];
endEst(99)=[];
%% Finding start of each cue1
cue1  = data(find(data(:,event) == predCue1on & data(:,1) > stimStart(1)), 1);
cue1Start = [cue1(1,1)];
previous = cue1(1,1);
for i = 1:size(cue1, 1)
%     Sometimes there is a 0 within the stimulus, we need to skip it
...
stimData = nan(2*nTrials*stimSize, size(data, 2) + 2);
stimNumber = [1:nTrials, 1:nTrials];
for i = 0: (2*nTrials-1)
    stimData(i*stimSize + 1:(i+1)*stimSize, 1:8) = data(data(:,1) >= stimStart(i+1) - stimBefore & ...
                                    data(:,1) < stimStart(i+1) + stimAfter, :);
...
predData = nan(2*(nTrials-1)*predSize, size(data, 2) + 2);
predNumber = [1:nTrials-1, 1:nTrials-1];
for j = 0: (2*nTrials-3)
    disp(j)
    predData(j*predSize + 1:(j+1)*predSize, 1:8) = data(data(:,1) >= predStart(j+1) - predBefore & ...
                                    data(:,1) < predStart(j+1) + predAfter, :);
...
%%
stimStartPred=stimStart;
stimStartPred(50:50:100)=[];
meanStimStartPred=mean(stimStartPred-predStart);

timeStampStim=ismember(unique(predData(:,1)),stimStartPred);


timeStampIndStim=find(timeStampStim==1);

timeStampPred=ismember(unique(predData(:,1)),predStart);

%keyboard

timeStampIndPred=find(timeStampPred==1);
meanStimStartPredInd=mean(timeStampIndStim-timeStampIndPred);
...
% close all
%dataCPtest = learningAndPerception_martin(sharedVar);
%learningAndPerception(vars);
...
firstStimCPpredSC=nan(1,nTrials-1);
secondStimCPpredSC=nan(1,nTrials-1);

firstStimOBpredSC=nan(1,nTrials-1);
secondStimOBpredSC=nan(1,nTrials-1);


for i=1:nTrials
    if i>1
        if alldata.chosenTargEst(i,1)==1
            firstStimCPpredSC(1,i-1)=dataCP1.surprise(1,i);
            secondStimCPpredSC(1,i-1)=dataCP2.surprise(1,i);
        else
            firstStimCPpredSC(1,i-1)=dataCP2.surprise(1,i);
            secondStimCPpredSC(1,i-1)=dataCP1.surprise(1,i);
        end
    end
...
for i=1:nTrials
    if i>1
        if alldata.chosenTargEst(i+50,1)==1
            firstStimOBpredSC(i-1)=dataOB1.surprise(1,i);
            secondStimOBpredSC(i-1)=dataOB2.surprise(1,i);
        else
            firstStimOBpredSC(i-1)=dataOB2.surprise(1,i);
            secondStimOBpredSC(i-1)=dataOB1.surprise(1,i);
        end
    end
...
% kl = [mean(klOB,1)]';
% surprise = [mean(surpriseOB,1)]';

kl = [mean(klCP,1),mean(klOB,1)]';
surprise = [mean(surpriseCP,1),mean(surpriseOB,1)]';

condNumCP=ones(length(klCP)-1,1);
condNumOB=ones(length(klOB)-1,1).*-1;
...
zArea = reshape(nanzscore(reshape(rArea, [2*(nTrials-1)*stimSize, 1])), [stimSize, 2*(nTrials-1)]);
...
zDiameter = reshape(nanzscore(reshape(diameter, [2*(nTrials-1)*stimSize, 1])), [stimSize, 2*(nTrials-1)]);
avgBaseline_d = mean(zDiameter(1:before,:));
avgBaseline_d_CP = mean(zDiameter(before+meanStimStartPredInd-200:before+meanStimStartPredInd,1:49));
avgBaseline_d_OB = mean(zDiameter(before+meanStimStartPredInd-200:before+meanStimStartPredInd,50:end));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
zDiffDiam=reshape(nanzscore(reshape(diffDiam, [2*(nTrials-1)*stimSize, 1])), [stimSize, 2*(nTrials-1)]);
...
xes = [ones(size(kl)), zscore(surprise), zscore(avgBaseline')];
xes_d = [ones(size(kl)), zscore(surprise),zscore(avgBaseline_d')];
xes_diff = [ones(size(kl)), zscore(surprise), zscore(avgBaseline_diff')];


%xes_CPpred=[ones(nTrials-1,1),zscore(firstStimCPpredSC)',zscore(secondStimCPpredSC)'];
%xes_OBpred=[ones(nTrials-1,1),zscore(firstStimOBpredSC)',zscore(secondStimOBpredSC)'];


xes_CPpred=[ones(nTrials-1,1),zscore(firstStimCPpredSC)',zscore(secondStimCPpredSC)',zscore(avgBaseline_d_CP')];
xes_OBpred=[ones(nTrials-1,1),zscore(firstStimOBpredSC)',zscore(secondStimOBpredSC)',zscore(avgBaseline_d_OB')];
...
%     [b_diff, bint_diff] = regress(zDiffDiam(i+stimBefore,:)', xes_diff);
    ...
    [b_dCP, bint_dCP] = regress(zDiameter(i+stimBefore,1:49)', xes_CPpred);
    [b_dOB, bint_dOB] = regress(zDiameter(i+stimBefore,50:end)', xes_OBpred);
    
    intercept_params_CP(i+stimBefore, 1)=bint_dCP(1,1);
    intercept_params_CP(i+stimBefore, 2)=bint_dCP(1,2);
    intercept_params_OB(i+stimBefore, 1)=bint_dOB(1,1);
    intercept_params_OB(i+stimBefore, 2)=bint_dOB(1,2);
   
    firstStimSC_params_CP(i+stimBefore, 1)=bint_dCP(2,1);
    firstStimSC_params_CP(i+stimBefore, 2)=bint_dCP(2,2);
    firstStimSC_params_OB(i+stimBefore, 1)=bint_dOB(2,1);
    firstStimSC_params_OB(i+stimBefore, 2)=bint_dOB(2,2);
    
    secondStimSC_params_CP(i+stimBefore, 1)=bint_dCP(3,1);
    secondStimSC_params_CP(i+stimBefore, 2)=bint_dCP(3,2);
    secondStimSC_params_OB(i+stimBefore, 1)=bint_dOB(3,1);
    secondStimSC_params_OB(i+stimBefore, 2)=bint_dOB(3,2);
    
    bes_intercept_CP(i+stimBefore, 1)=b_dCP(1);
    bes_first_CP(i+stimBefore, 1)=b_dCP(2);
    bes_second_CP(i+stimBefore, 1)=b_dCP(3);
   
    bes_intercept_OB(i+stimBefore, 1)=b_dOB(1);
    bes_first_OB(i+stimBefore, 1)=b_dOB(2);
    bes_second_OB(i+stimBefore, 1)=b_dOB(3);
    
%     intercept_params(i+stimBefore, 1) = bint(1,1);
%     intercept_params(i+stimBefore, 2) = bint(1,2);
%     
%     surprise_params(i+stimBefore, 1) = bint(2, 1);
%     surprise_params(i+stimBefore, 2) = bint(2, 2);
%     
% %     kl_params(i+stimBefore, 1) = bint(3,1);
% %     kl_params(i+stimBefore, 2) = bint(3,2);
% %     bes_kl(i+stimBefore) = b(3);
%     
%     bes_intercept(i+stimBefore) =b(1);
%     
%     bes_surprise(i+stimBefore) = b(2);
%     
%     
%     intercept_params_d(i+stimBefore, 1) = bint_d(1,1);
%     intercept_params_d(i+stimBefore, 2) = bint_d(1,2);
%    
%     surprise_params_d(i+stimBefore, 1) = bint_d(2, 1);
%     surprise_params_d(i+stimBefore, 2) = bint_d(2, 2);
%     bes_intercept_d(i+stimBefore) =b_d(1);
%     
% %     kl_params_d(i+stimBefore, 1) = bint_d(3,1);
% %     kl_params_d(i+stimBefore, 2) = bint_d(3,2);
% %     bes_kl_d(i+stimBefore) = b_d(3);
%     
%     
%     bes_surprise_d(i+stimBefore) =b_d(2);
%     
%     
%     intercept_params_diff(i+stimBefore, 1) = bint_diff(1,1);
%     intercept_params_diff(i+stimBefore, 2) = bint_diff(1,2);
%    
%     surprise_params_diff(i+stimBefore, 1) = bint_diff(2, 1);
%     surprise_params_diff(i+stimBefore, 2) = bint_diff(2, 2);
%     bes_intercept_diff(i+stimBefore) =b_diff(1);
%    
%     bes_surprise_diff(i+stimBefore) =b_diff(2);
%     
% %     kl_params_diff(i+stimBefore, 1) = bint_diff(3,1);
% %     kl_params_diff(i+stimBefore, 2) = bint_diff(3,2);
% %     bes_kl_diff(i+stimBefore) = b_diff(3);
%     
...
for i = -stimBefore+1:stimAfter
    ...
    means_d(i + stimBefore) = nanmean(zDiameter(i+stimBefore,:));
    means_diff(i + stimBefore) = nanmean(zDiffDiam(i+stimBefore,:));
    
end

sterr =  std(zArea') / ((nTrials-1)*2)^(.5);
sterr_d =  std(zDiameter') / ((nTrials-1)*2)^(.5);
sterr_diff =  std(zDiffDiam') / ((nTrials-1)*2)^(.5);

trialSC=alldata.surprise;
trialSCcp=mean(trialSC(1:50,:),2);
trialSCob=mean(trialSC(51:end,:),2);

trialSCcp(trialSCcp>0)=1;
trialSCob(trialSCob>0)=-1;

trialSCcp(1)=[];
trialSCob(1)=[];

trialSC=[trialSCcp;trialSCob];

trial_CP=find(trialSC==1);
trial_OB=find(trialSC==-1);
trial_neutral=find(trialSC==0);


for i = -stimBefore+1:stimAfter
    means_d_CP(i + stimBefore) = nanmean(zDiameter(i+stimBefore,trial_CP));
    
    means_d_OB(i + stimBefore) = nanmean(zDiameter(i+stimBefore,trial_OB));
    
    means_d_neutral(i + stimBefore) = nanmean(zDiameter(i+stimBefore,trial_neutral));
    
end

diamCP=zDiameter(:,trial_CP);
diamOB=zDiameter(:,trial_OB);
diamN=zDiameter(:,trial_neutral);


sterr_d_CP =  std(diamCP') / ((nTrials-1)*2)^(.5);
sterr_d_OB =  std(diamOB') / ((nTrials-1)*2)^(.5);
sterr_d_neutral =  std(diamN') / ((nTrials-1)*2)^(.5);
...
result.kl_confidence = kl_params;
result.surprise_confidence = surprise_params;
result.bes_kl = bes_kl;
result.bes_surprise = bes_surprise;
...
result.diameterMatrix = zDiameter;
result.kl_confidence_d = kl_params_d;
result.surprise_confidence_d = surprise_params_d;
result.bes_kl_d = bes_kl_d;
result.bes_surprise_d = bes_surprise_d;
...
result.means_d_CP = means_d_CP';
result.means_d_OB = means_d_OB';
result.means_d_neutral = means_d_neutral';
result.sterr_d_CP = sterr_d_CP;
result.sterr_d_OB = sterr_d_OB;
result.sterr_d_neutral = sterr_d_neutral;


result.stimBefore = before;
result.stimAfter = after;
result.meanEndStim=meanEndStim;
result.meanEndEst=meanEndEst;


result.intercept_confidence_CP=intercept_params_CP;
result.intercept_confidence_OB=intercept_params_OB;
result.firstStimSC_confidence_CP=firstStimSC_params_CP;
result.secondStimSC_confidence_CP=secondStimSC_params_CP;
result.firstStimSC_confidence_OB=firstStimSC_params_OB;
result.secondStimSC_confidence_OB=secondStimSC_params_OB;

result.bes_intercept_CP=bes_intercept_CP;
result.bes_intercept_OB=bes_intercept_OB;
result.bes_first_CP=bes_first_CP;
result.bes_first_OB=bes_first_OB;
result.bes_second_CP=bes_second_CP;
result.bes_second_OB=bes_second_OB;
